...
PikaPort::~PikaPort() {
  ...
  DLOG(INFO) << "PikaPort " << pthread_self() << " exit!!!";
}

bool PikaPort::Init() {
  // DLOG(INFO) << "host: " << g_port_conf.local_ip << " port: " << g_port_conf.local_port;
  return true;
}
...
void PikaPort::Start() {
  ...
  // if (g_port_conf.filenum >= 0 && g_port_conf.filenum != UINT32_MAX && g_port_conf.offset >= 0) {
  if (g_port_conf.filenum != UINT32_MAX) {
    logger_->SetProducerStatus(g_port_conf.filenum, g_port_conf.offset);
  }
...
bool PikaPort::SetMaster(std::string& master_ip, int master_port) {
  ...
  if ((role_ ^ PIKA_ROLE_SLAVE) && repl_state_ == PIKA_REPL_NO_CONNECT) {
    ...
    DLOG(INFO) << "set role_ = PIKA_ROLE_PORT, repl_state_ = PIKA_REPL_CONNECT";
    return true;
  }
  return false;
}
